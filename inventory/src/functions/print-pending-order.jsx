export function generatePrintPendingOrder(order, userData, printDate, calculateTotalPrice, paymentType) {
    return `
      <html lang="en">
        <head>
          <meta charset="UTF-8">
        ...
        <body>
          ...
          <p>Payment Type(s)</p>
          ${Object.entries(paymentType).map(([type, amount]) => `
          ...
          <div class="totals">
            <p><strong>Total:</strong> ₦${new Intl.NumberFormat().format(calculateTotalPrice(order.orderedItems))}.00</p>
            <p><strong>Discount:</strong> ₦${order.discount || "0.00"}</p>
            <p><strong>Paid:</strong> ₦${new Intl.NumberFormat().format(order.amountPaid || calculateTotalPrice(order.orderedItems))}.00</p>
